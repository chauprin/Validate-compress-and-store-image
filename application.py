...
#UPLOAD_FOLDER='/home/ubuntu/dev/bic2/static/uploads/'
ALLOWED_EXTENSIONS=set(['png','jpeg','gif','webp','xmb','bmp','ico'])


app.config.update(
	DEBUG = True,
	UPLOADS_DEFAULT_URL = UPLOAD_FOLDER,
...
def upload():
    ...
    try:
        ...
        url=data['url']
        url=url.split("/")
        url="/"+"/".join(url[url.index('static'):])

        return render_template("view.html" , filename=data['filename'], url=url)
